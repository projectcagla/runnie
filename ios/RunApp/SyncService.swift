...
public final class SyncService: ObservableObject {
    ...
    @Published public var serverBaseUrl: String = "http://localhost:3000"
    ...
    public func registerDeviceIfNeeded() async throws {
        ...
        guard let url = URL(string: "\(serverBaseUrl)/v1/auth/device-register") else { return }
    ...
    public func syncWorkouts(_ payloads: [NormalizedActivityPayload]) async throws -> [[String: Any]] {
        ...
        guard let url = URL(string: "\(serverBaseUrl)/v1/sync/activities") else { return [] }
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
